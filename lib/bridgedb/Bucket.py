...
class BucketManager:
    ...
    def dumpBridgesToFile(self, filename, bridges):
        ...
        with bridgedb.Storage.getDB() as db:
            ...
            try:
                logging.debug("Dumping bucket to %s" % filename)
                f = open(filename, 'w')
    ...
    def dumpBridges(self):
        ...
        """
        logging.debug("Dumping all bridges to file, per distributor")
        with bridgedb.Storage.getDB() as db:
            allBridges = db.getAllBridges()
        bridgeDict = {}
        # Sort returned bridges by distributor
        logging.debug("%d bridges, in total" % len(allBridges))
        for bridge in allBridges:
        ...
        # Now dump to file(s)
        logging.debug("Distributor Buckets: %s" % bridgeDict.keys())
        for k in bridgeDict.keys():
            dist = k
            logging.debug("Dumping %s bridges" % dist)
